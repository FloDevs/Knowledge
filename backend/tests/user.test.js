...
describe("User Model", () => {
  ...
  it("ne doit pas autoriser deux utilisateurs avec le même email", async () => {
    const user1 = new User({ name: "User1", email: "duplicate@example.com", password: "123456" });
    await user1.save();

    try {
      const user2 = new User({ name: "User2", email: "duplicate@example.com", password: "abcdef" });
      await user2.save();
